...
`adapters/clouddrive/` 里的 `clouddrive_pb2*.py` 是从 CloudDrive2 proto
生成后 vendor 进来的（连带 grpcio/protobuf 两个依赖——对"最小依赖"姿态的
一次有意偏离）；不要手改生成文件。gRPC 只在这个包内出现，上层一律通过
`AsyncCloudDrive` 与稳定错误码交互。
