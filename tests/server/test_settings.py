...
def test_settings_require_explicit_dsn_and_one_worker(tmp_path: Path) -> None:
    environ = {
        "REELOOM_POSTGRES_DSN": "postgresql://app@db/reeloom",
        "REELOOM_MIGRATION_POSTGRES_DSN": (
            "postgresql://migrator@db/reeloom"
        ),
    ...
    assert (
        settings.migration_postgres_dsn
        == environ["REELOOM_MIGRATION_POSTGRES_DSN"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"REELOOM_MIGRATION_POSTGRES_DSN": ""},
            ServerErrorCode.INVALID_SETTINGS,
        ),
