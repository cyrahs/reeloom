...
| | V1 | V2 |
| --- | --- | --- |
| Python 源码 | 67,529 行 | 4,927 行 |
| 前端 | 6,024 行 | 1,189 行 |
| 最大单文件 | 2,892 行 | 512 行（`server/api.py`） |
| 测试 | — | 3,636 行 / 213 个用例 |

实际实现与计划的三处偏差，都是构建过程中发现的真实缺陷：

1. **扫描回环**：失败 run 留下的文件夹会被无限重开。修复方式是与上一个
   run 的 snapshot 做内容比较，而不是加状态标记。
2. **未配置即失败**：凭据没填时每个发现的文件夹都会被判 FAILED，配置好后
   也不会自动恢复。新增 `Deferred` 错误，这类 run 退回 `pending` 等待唤醒，
   不计入 attempts。
3. **AT_FDCWD 平台差异**：Linux 是 -100、Darwin 是 -2，原先只因为路径都是
   绝对路径才碰巧正确。现在显式按平台取值并要求绝对路径。

字幕语言判定从 Agent 工具改为扫描时写入 snapshot，因此 `planner.py` 保持为
纯函数，模型也无法影响字幕编码判定。
