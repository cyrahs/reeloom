...
| Admin config → filesystem/network capability | Agent-selected paths or URL, secret disclosure, config drift | admin-only versioned CAS, exact config revision bound to every run, authorized absolute roots, write-only no-follow SecretStore, provider origin allowlist |
| Provider/TMDB network | redirect, environment proxy, DNS rebinding, arbitrary Agent HTTP | pinned resolved provider origin, redirects and environment proxy disabled, bounded transport; TMDB is the sole business network adapter and never an Agent URL tool |
